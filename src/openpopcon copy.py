...
# Boolean flag for stellarator and tokamak
import argparse
p = argparse.ArgumentParser() # Creates a new ArgumentParser object and stores it in p
p.add_argument("--stellarator", action = 'store_true', help = "Use stellarator mode (else tokamak).")
    # This adds one optional flag to the form: --stellarator.
    # action='store_true' means:
        # If the user includes --stellarator on the command line, argparse will set args.stellarator = True.
        # If the user omits it, argparse will set args.stellarator = False.
        # So it’s a simple on/off switch.
    # help="..." is the short description that shows up when someone runs your script with --help.
args = p.parse_args()
    # This reads the real command line (what the user typed) and returns a Namespace object.
    # args is that object; each argument becomes an attribute on it.
    # In this case, you’ll get args.stellarator as either True or False depending on whether the flag was present.
...
import numba as nb
    ...
    # speeds up numerical functions by translating Python bytecode to optimized machine code for both CPUs and GPUs
...
if arg.stellarator:
    ...
    # ['resources','scalinglaws.yml']
        # This is a Python list of path parts. Think: folder "resources" and file "scalinglaws.yml"
    # get_POPCON_homedir(...)
        # This is a function call (defined somewhere in OpenPOPCON) to make a full absolute file path.
...
class POPCON_algorithms:
    ...
    if args.stellarator:
        ...
        def tauE_scalinglaw(self, Pheat, n_e_20) -> float: # Change the following to a more universal scaling law 
            """
    ...
    def P_aux_relax_impfrac(self, n_e_20, T_i_keV, accel=1., err=1e-5, max_iters=1000):
        ...
        for ii in np.arange(max_iters):
            ...
            # Power out
            tauE_iter = self.tauE_scalinglaw(P_totalheating_iter, n_e_20*line_average_fac) # TO CHANGE
