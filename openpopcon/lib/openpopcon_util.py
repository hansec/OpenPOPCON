...
def example_dir(name: str) -> str:
    ...
    an editable copy.
    """
    exdir = importlib.resources.files("openpopcon").joinpath(
...
def read_eqdsk(filename):
    ...
    with open(filename, "r") as fid:
        ...
        # Read PSI grid
        eqdsk_obj["psirz"] = read_2d(fid, eqdsk_obj["nz"], eqdsk_obj["nr"])
        # Read q-profile
        ...
        R = np.linspace(
            eqdsk_obj["rleft"], eqdsk_obj["rleft"] + eqdsk_obj["rdim"], eqdsk_obj["nr"]
        )
        ...
        fluxorder = slice(
            None, None, 1 - 2 * (eqdsk_obj["psimag"] > eqdsk_obj["psibry"])
        )
        fpolrz = np.interp(
            eqdsk_obj["psirz"], psi1d[fluxorder], eqdsk_obj["fpol"][fluxorder]
        )
...
def get_contours(gEQDSK: dict, Npsi: int = 50):
    ...
    """
    rs = np.linspace(
        gEQDSK["rleft"], gEQDSK["rleft"] + gEQDSK["rdim"], gEQDSK["psirz"].shape[1]
    )
    zs = np.linspace(
    ...
    for level in psin:
        ...
        if len(enclosing) != 0:
            ...
            chosen = min(
                segset,
                key=lambda seg: np.hypot(
                    np.mean(seg[:, 0]) - axis_point[0],
...
def get_current_density(gEQDSK: dict, Npsi: int = 50):
    ...
    zs = np.linspace(
        gEQDSK["zmid"] - 0.5 * gEQDSK["zdim"], gEQDSK["zmid"] + 0.5 * gEQDSK["zdim"], nz
    )
    ...
    fprimrz = np.interp(
        psirz, psi[fluxorder], np.gradient(gEQDSK["fpol"][fluxorder], psi[fluxorder])
    )
    ...
    for i, level in enumerate(psin):
        ...
        javgrms[i] = np.sqrt(
            np.sum(weights * jtotspline.ev(zmid, rmid) ** 2) / totalweight
        )
